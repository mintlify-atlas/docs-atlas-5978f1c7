---
title: "ACME DNS-01: Automated TLS Certificate Issuance Flow"
sidebarTitle: "ACME DNS-01"
description: "Explore how DNS Handling implements RFC 8555 ACME DNS-01 challenges with the acmez/v3 library to automatically issue TLS certificates from Let's Encrypt."
---

DNS Handling implements the ACME protocol (RFC 8555) DNS-01 challenge type using the [`acmez/v3`](https://github.com/mholt/acmez/v3) library to automatically issue TLS certificates from Let's Encrypt or any compatible ACME certificate authority. The process is entirely asynchronous: the API returns immediately after starting an order and a background goroutine handles the polling, challenge completion, and certificate storage without blocking the caller.
...
<Steps>
  ...
  <Step title="StartOrder — create the ACME order and extract the DNS-01 challenge">
    ...
    The required DNS TXT value is then computed from the challenge token and the account key using `acmez/v3`:
